...
type Dispatcher struct {
	...
	Workers         chan *Worker
	maxWorkers      int
	jobsQueue       chan UbkJob
	unperformedJobs []UbkJob
	stop            chan bool
	stopped         chan bool
	wg              *sync.WaitGroup
	stopInProgress  bool
...
func NewDispatcher(maxWorkers int, jobsQueueSize int) *Dispatcher {
	jobsQueue := make(chan UbkJob, jobsQueueSize)
	var unperformedJobs []UbkJob // when stop copy all jobs from jobsQueue to unperformedJobs
	pool := make(chan chan UbkJob, maxWorkers)
...
func (d *Dispatcher) AddJob(job UbkJob) {
...
// GetUnperformedJobs method returns a chan of UbkJob that have not been done before Stop() executed
func (d *Dispatcher) GetUnperformedJobs() []UbkJob {
	return d.unperformedJobs
}

// CleanUnperformedJobs remove unperformedJobs
func (d *Dispatcher) CleanUnperformedJobs() {
	d.unperformedJobs = make([]UbkJob, 0)
}
